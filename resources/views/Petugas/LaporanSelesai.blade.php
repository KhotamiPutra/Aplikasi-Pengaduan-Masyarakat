...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <h2 class="text-3xl font-extrabold mb-10 text-[#D97757] text-center tracking-tight">Laporan Selesai</h2>
                    ...
                    <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-[#f3e7e2] flex flex-col h-full transition hover:shadow-2xl">
                        ...
                        <div class="p-6 flex flex-col flex-1">
                            <div class="mb-2 flex items-center gap-2">
                                ...
                                <span class="text-xs text-gray-400 ml-auto">{{ $putra_pengaduan_item->tgl_pengaduan }}</span>
                            </div>
                            <div class="mb-3">
                                <span class="block text-sm font-semibold text-[#D97757]">Isi Laporan:</span>
                                <p class="text-base text-gray-800 mt-1">{{ Str::limit($putra_pengaduan_item->isi_laporan, 70) }}</p>
                            </div>
                            <div class="mb-2 grid grid-cols-2 gap-2">
                                <div>
                                    <span class="block text-xs font-semibold text-gray-500">NIK</span>
                                    <span class="block text-sm text-gray-800">{{ $putra_pengaduan_item->putra_masyarakat->nik ?? '-' }}</span>
                                </div>
                                <div>
                                    <span class="block text-xs font-semibold text-gray-500">Username</span>
                                    <span class="block text-sm text-gray-800">{{ $putra_pengaduan_item->putra_masyarakat->username ?? '-' }}</span>
                                </div>
                            </div>
                            <div class="flex-1"></div>
                            <div class="flex justify-end mt-6 gap-2">
                                <a href="{{ route('Petugaslaporan.tanggapan.create', $putra_pengaduan_item->id_pengaduan) }}"
