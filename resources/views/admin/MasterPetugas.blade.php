...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
            <h2 class="text-2xl font-bold text-[#D97757] mb-2 md:mb-0">Data Petugas</h2>
            <div class="flex flex-col md:flex-row md:items-center gap-2">
                <form method="GET" action="{{ route('cari.index') }}" class="flex gap-2">
                ...
                <a href="{{ route('petugas.create') }}" class="px-4 py-2 bg-[#D97757] text-white rounded-lg hover:bg-[#bf6443] transition font-semibold shadow">
                    + Tambah Petugas
                </a>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-[#f3e7e2]">
                <thead class="bg-[#f9f6f5]">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold text-[#D97757] uppercase tracking-wider">Level</th>
                ...
                <tbody class="bg-white divide-y divide-[#f3e7e2]">
                    @foreach ($putra_petugas as $putra_m)
                        <tr class="hover:bg-[#D97757]/10 transition">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $putra_m->nama_petugas }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $putra_m->level }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <a href="{{ route('petugas.edit', $putra_m->id_petugas) }}" class="inline-flex items-center justify-center bg-[#D97757]/10 hover:bg-[#D97757]/20 rounded-full p-2 transition" title="Edit">
                                    <img src="{{ asset('asset/images/pen (1).png') }}" alt="Edit" class="h-5 w-5" />
                                </a>
                                <form action="{{ route('petugas.destroy', $putra_m->id_petugas) }}" method="POST" class="inline"
