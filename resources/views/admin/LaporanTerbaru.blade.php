...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <h2 class="text-3xl font-extrabold mb-10 text-[#D97757] text-center tracking-tight">Laporan Terbaru</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach ($putra_pengaduan as $pengaduan)
                    <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-[#f3e7e2] flex flex-col h-full transition hover:shadow-2xl">
                        @if ($pengaduan->foto)
                            <img src="{{ asset('storage/' . $pengaduan->foto) }}" alt="Foto Laporan" class="h-44 w-full object-cover rounded-t-2xl">
                        @else
                        ...
                        <div class="p-6 flex flex-col flex-1">
                            ...
                            <div class="flex justify-between mt-6 gap-2">
                                <a href="{{ route('laporan.konfirmasi', $pengaduan->id_pengaduan) }}"
                                   class="bg-[#22c55e] hover:bg-[#16a34a] text-white px-4 py-2 rounded-lg font-semibold text-xs shadow transition">
                                    Proses Laporan
                                </a>
                                <a href="{{ route('laporanMasyarakat.hapus', $pengaduan->id_pengaduan) }}"
                                   class="bg-[#ef4444] hover:bg-[#dc2626] text-white px-4 py-2 rounded-lg font-semibold text-xs shadow transition"
