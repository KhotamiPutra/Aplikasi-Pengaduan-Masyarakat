                        ...
                        <div class="p-4">
                            ...
                            <div class="mb-2">
                                ...
                                @if ($putra_pengaduan_item->foto)
                                    <img src="{{ asset('storage/' . $putra_pengaduan_item->foto) }}" alt="Foto Laporan"
                                        class="mt-1 h-40 w-full object-cover rounded">
                                @else
                            ...
                            <div class="mb-2">
                                ...
                                <span
                                    class="text-sm text-gray-800 ml-2">{{ $putra_pengaduan_item->putra_masyarakat->nik ?? '-' }}</span>
                            </div>
                            <div class="mb-4">
                                <span class="text-sm font-semibold text-gray-600">Username:</span>
                                <span
                                    class="text-sm text-gray-800 ml-2">{{ $putra_pengaduan_item->putra_masyarakat->username ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <a href="{{ route('laporanSelesai.index', ['id_laporan' => $putra_pengaduan_item->id_pengaduan]) }}"
                                    class="bg-green-500 text-white px-3 py-1 rounded hover:bg-green-600 text-sm">Selesai</a>
                                <a href="{{ route('laporan.tanggapan.create', $putra_pengaduan_item->id_pengaduan) }}"
                                    class="bg-[#D97757] text-white px-3 py-1 rounded hover:bg-[#bf6443] text-sm">Tanggapan</a>
                            </div>
                        </div>
                    </div>
                @endforeach
